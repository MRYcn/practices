...
class Block_game:
                                ...
                                if event.type==pygame.MOUSEBUTTONDOWN:
                                        ...
                                        for block in self.c_blocks:
                                                if block.rect.collidepoint(mouse_pos):
                                                        self.status=True
                                                        self.stats.level=_update_level(n)
        ...
        def _create_intro(self):
                ...
                self.intro_image_rect.midtopx=600
                self.intro_image_rect.midtopy=450
        ...
        def _create_block(self):
                if status:
                        if self.bl.d1:
                                ...
                                self.bl.b1=datetime.datetime.now()
                else:
                        x,y=200,350
                        self.c_blocks.empty()
                        for i in range(5):
        ...
        def _update_level(self,n):
                ...
                while x<1170 and y>70:
                        x+=x_r
                        y+=y_r
                        level.rect.center.x=x
                        level.rect.center.y=y
                        level.change_size()
        ...
        def _update_screen(self):
                self.screen.fill(self.settings.bg_color)
                if not status:
                        self.high_levels.blitme()
#           for block in self.c_blocks.sprites():
                        self.c_blocks.blitme()
                        self.levels.blitme()
